...
init python:
    ...
    def sms(txt):
        ...
        if ("{#l}" in txt):
            renpy.music.play("code/sms/send.mp3", channel="sound", loop=False, fadein=0, fadeout=0)
        if ("{#r}" in txt):
            renpy.music.play("code/sms/send.mp3", channel="sound", loop=False, fadein=0, fadeout=0)
    ...
    frame:
        ...
        vbox:
            ...
            for i in sms_text:
                ...
                textbutton _("{size=20}{font=code/sms/SFRegular.ttf}" + i + "{/font}{/size}"):
                    xminimum 60
                    yminimum 60
                    top_padding pp
                    bottom_padding pp
                    ...
                    if "{#r}" in i:
                        xalign 0.55
                        background Frame("code/sms/smsright.png", 60, 38)
                    else:
                        xalign 0.0
                        background Frame("code/sms/smsleft.png", 60, 38)
                    action []
        ...
        add "code/sms/smscaption.png" align(-.045, .0) yoffset -115
